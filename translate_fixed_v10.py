#!/usr/bin/env python3
"""Tales of Destiny 2 JP->EN translation pipeline for llama.cpp llama-server.

Standard-library only. Designed for OpenAI-compatible llama-server endpoints.
It processes divider-delimited .sced.txt blocks, skips already translated blocks,
protects script codes, uses exact-length JSON output when compatible, falls back
safely when llama.cpp grammar conflicts with reasoning tokens, repairs placeholders
deterministically, and writes translations after Japanese lines.
"""
...
- Use glossary translations exactly when applicable.
- Preserve tone; do not censor or invent content.
- Each returned string must be a single non-empty English line.
- Keep extended cries, groans, laughs, and repeated punctuation concise. Never repeat one character more than 8 times; use forms like "Mmm..." or "Aaaah!" instead of a long character run.
...
def call_llama_server(
    ...
    *,
    ...
    automatic_cap = max(args.line_max_tokens, expected_lines * args.tokens_per_line)
    ...
    for attempt in range(1, args.retries + 1):
        ...
        try:
            ...
            runaway = runaway_output_reason(
                content, finish_reason, args.max_line_chars, expected_lines
            )
            if runaway:
            ...
            for line in lines:
                if len(line) > args.max_line_chars:
                    raise ValueError(
                        f"translation line is too long ({len(line)} characters; "
                        f"limit {args.max_line_chars})"
                    )
    ...
    def translate_one(target_index: int, retry_reason: Exception | None = None) -> str:
        ...
        try:
            return call_llama_server(
                ...
                1,
                args,
                max_tokens_override=args.line_max_tokens,
            )[0]
...
- Give a short, ordinary English translation.
- If the source is a prolonged sound or vocalization, shorten it naturally, for example "Mmm...", "Aaaah!", or "Grrr...".
...
"""
            return call_llama_server(
                ...
                1,
                args,
                max_tokens_override=args.rescue_max_tokens,
                rescue_mode=True,
            )[0]

    try:
        translated = call_llama_server(prompt, len(protected_lines), args)
    ...
    for block_no, block in enumerate(blocks):
        ...
        try:
            ...
            if not args.dry_run:
                ...
                translated_count += 1
                # Checkpoint every completed block. Scenario files can contain
                # hundreds of blocks, so a late failure must not discard earlier work.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    scope.add_argument("--file", type=Path, help="translate one .sced.txt file")
    ...
    parser.add_argument("--tokens-per-line", type=int, default=int(os.getenv("LLM_TOKENS_PER_LINE", "128")),
                        help="dynamic block output budget per source line")
    parser.add_argument("--line-max-tokens", type=int, default=int(os.getenv("LLM_LINE_MAX_TOKENS", "128")),
                        help="output-token ceiling for a single-line fallback")
    parser.add_argument("--rescue-max-tokens", type=int, default=int(os.getenv("LLM_RESCUE_MAX_TOKENS", "96")),
                        help="small ceiling for anti-repetition rescue generation")
    ...
    parser.add_argument("--max-line-chars", type=int, default=int(os.getenv("LLM_MAX_LINE_CHARS", "300")),
                        help="reject implausibly long translation lines")
...
def main() -> int:
    ...
    if args.file:
        ...
        scenario_dir = args.root / "ps2" / "scenarios"
        skit_dir = args.root / "ps2" / "skits"
        dirs = [scenario_dir] if args.scenarios_only else [skit_dir] if args.skits_only else [scenario_dir, skit_dir]
        files = sorted(p for directory in dirs if directory.exists() for p in directory.rglob("*.sced.txt"))
